...
async function berekenScoreVoorSpeler(sessie, spelerId) {
  ...
  const actieveRennerIds = draft
    .filter(
      (keuze) =>
        Number(keuze.speler_id) === Number(spelerId) && keuze.is_bank === false,
    )
    .map((keuze) => Number(keuze.renner_id));

  return (ritresultaten || [])
    .filter((resultaat) =>
      actieveRennerIds.includes(Number(resultaat.renner_id)),
    )
...
async function berekenTruiWinnaars(wedstrijdId) {
...
async function telTruiWinnaarsUitRitten(ritten) {
  ...
  const naamMap = (renners || []).reduce((map, renner) => {
    map[renner.slug] = renner.naam;
    return map;
  }, {});
  ...
  return truiTypes.reduce(
    (result, { key }) => {
      ...
      const [bestSlug, besteCount] = items.reduce(
        (top, current) => (current[1] > top[1] ? current : top),
        ["", 0],
      );

      result[key] = {
        naam: naamMap[bestSlug] || bestSlug || "-",
        aantal: besteCount,
      };
...
router.get("/me", async (req, res) => {
  try {
    ...
    if (actieveSessies && actieveSessies.length > 0) {
      ...
      var topTruien = await berekenTruiWinnaars(sessie.wedstrijd_id);
    }
