...
const maakCompetitie = async (req, res) => {
  ...
  try {
    ...
    const { error: spelerError } = await supabase.from("spelers").insert([
      {
    ...
    const { error: sessieError } = await supabase.from("draft_sessies").insert([
      {
        Naam: `Draft ${naam}`,
        competitie_id: competitie.id,
        is_actief: true,
    ...
    res
      .status(500)
      .json({ error: "Kon competitie niet aanmaken", details: error.message });
  }
};
...
const joinCompetitie = async (req, res) => {
  ...
  try {
    ...
    if (bestaandeSpeler) {
      return res
        .status(400)
        .json({ error: "Je doet al mee aan deze competitie." });
    }
